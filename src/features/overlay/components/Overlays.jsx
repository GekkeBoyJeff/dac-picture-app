...
/* eslint-disable @next/next/no-img-element -- Native img required for precise overlay DOM measurement used by compositePhoto. */
...
} from "@/features/overlay/store"
import { useBreakpoint, resolveBreakpoint as bp } from "@/features/overlay/hooks/useBreakpoint"
...
export const Overlays = memo(function Overlays() {
  ...
  const { tier, orientation } = useBreakpoint()
  ...
  return (
    <>
      ...
      >
        <span
          className="text-white/90 font-bold tracking-[0.15em] uppercase leading-tight"
      ...
      <span
        data-overlay="date"
        className="absolute text-white/70 font-mono pointer-events-none z-10 left-1/2 -translate-x-1/2"
        style={{
