...
/**
 * Palm-swipe tracking for the layout slider.
 *
 * After Open_Palm is held for GESTURE_SWIPE_ENGAGE_MS the swipe becomes
 * engaged and delta tracking begins from the anchor wrist position.
 * A dead zone prevents jitter, and the consumer can use GESTURE_SWIPE_SNAP_THRESHOLD
 * externally to decide when to snap to the next/previous layout.
 *
...
export function useGestureSwipe(rawGestureNameRef, primaryHandLandmarksRef, { enabled = false }) {
  ...
  const tick = useCallback(() => {
    ...
    // Use landmark 9 (middle finger MCP) as the palm centre proxy
