...
export function useOverlaySettings() {
  ...
  const layout: LayoutPreset = LAYOUTS.find((l) => l.id === layoutId) || LAYOUTS[0];
  const mascot: Mascot = MASCOTS.find((m) => m.id === mascotId) || MASCOTS[0];
  const activeConvention: Convention | null = getActiveConvention();
