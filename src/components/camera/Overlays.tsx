...
import type { OverlaySize, OverlayPosition } from "@/lib/types";
...
function useBreakpoint(): Breakpoint {
  return useSyncExternalStore(subscribe, getBreakpoint, () => "lg" as Breakpoint);
}

function rem(v: number): string {
  return `${v}rem`;
}
...
function positionStyle(
  ...
  inset: number,
  size: OverlaySize,
  opts?: { opacity?: number; fixedSize?: boolean; padding?: number },
): React.CSSProperties {
  if (position === "full") {
...
export function Overlays({ layout, mascot, activeConvention }: OverlaysProps) {
  ...
  return (
    <>
      ...
      {CORNERS.map((c) => {
        ...
        const style: React.CSSProperties = { width: rem(cornerSize), height: rem(cornerSize) };
