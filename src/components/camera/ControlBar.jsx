...
import { FullscreenIcon, DownloadIcon, GridIcon, MascotIcon, LayoutIcon, CameraSwitchIcon, SettingsIcon } from "@/components/ui/icons"
  ...
  return (
    <div className="pointer-events-auto absolute right-3 top-1/2 -translate-y-1/2 flex flex-col gap-2 max-lg:landscape:right-auto max-lg:landscape:top-auto max-lg:landscape:translate-y-0 max-lg:landscape:bottom-3 max-lg:landscape:left-1/2 max-lg:landscape:-translate-x-1/2 max-lg:landscape:flex-row z-10">
      <div className="relative group">
        <button onClick={() => openModal("gallery")} className={`relative ${BUTTON_STYLES.icon}`} aria-label="Gallery">
          <GridIcon className="w-5 h-5 text-white/70" />
          {galleryCount > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-white/30 backdrop-blur-sm text-white text-[0.625rem] flex items-center justify-center font-medium">
              {galleryCount}
            </span>
          )}
        </button>
      ...
      <div className="relative group">
        <button onClick={() => openModal("mascotPicker")} className={BUTTON_STYLES.icon} aria-label="Choose mascot">
      ...
      <div className="relative group">
        <button onClick={() => openModal("layoutSlider")} className={BUTTON_STYLES.icon} aria-label="Choose layout">
      ...
      {devices.length > 0 && (
        <div className="relative">
          <div className="relative group">
            <button onClick={toggleCameraMenu} className={BUTTON_STYLES.icon} aria-label="Camera wisselen">
      ...
      <div className="relative group hidden md:block">
        <button onClick={toggleFullscreen} className={`${BUTTON_STYLES.icon} hidden md:flex`} aria-label="Fullscreen">
      ...
      <div className="relative group">
        <button onClick={() => openModal("settings")} className={BUTTON_STYLES.icon} aria-label="Instellingen">
